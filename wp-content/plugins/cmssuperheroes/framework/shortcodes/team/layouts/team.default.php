...
<article <?php  post_class(); ?>>
    ...
    <div class="cshero-team-content" <?php echo $content_style;?>>
        ...
        <<?php echo $item_heading_size;?> class="cshero-team-title" <?php echo $team_title_style;?>>
                <a style="color:<?php echo $title_team_color?> ;" title="<?php the_title(); ?>" href="<?php the_permalink() ?>" rel=""><?php the_title() ?></a>
        ...
        <div class="cshero-team-info-wrap">
            ...
            <?php  if($show_team_position) {                            
                ...
                if ($team_position) { ?>
                <div class="cshero-team-info cshero-team-position"><i class="fa fa-users"></i> <?php echo strip_tags ($team_position); ?></div>
            <?php } } ?>
            <?php  ?>
        ...
        <?php if ($show_description) { ?>
            <div class="cshero-team-description"><?php echo substr(get_the_content($read_more), 0, $excerpt_length); ?></div>
        <?php } ?>

        <?php if(!$show_image) : ?>
            <?php if ($show_socials && !empty($links)) {
                $social_title = get_post_meta( $post->ID, '_cshero_team_social', true );
                if(!empty($social_title)){
                    echo '<h3 class="cshero-content-header">'.$social_title.'</h3>';
                }
