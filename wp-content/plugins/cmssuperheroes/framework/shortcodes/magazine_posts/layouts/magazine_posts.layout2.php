<div class="<?php echo str_replace('.','-',$layout); ?>">
	...
	if($wp_query->have_posts()){
		...
		while($wp_query->have_posts()): $wp_query->the_post();
			if($count==$start){
				?>
				<div class="magazine-post-featured">
					<article <?php post_class(' post-list-feature'); ?>>
	                    <header class="cs-blog-header">
	                        ...
	                        <div class="cs-blog-meta cs-itemBlog-meta">
	                            ...
	                            ?>
	                            <h5 class="feature-title"><a title="<?php the_title(); ?>" href="<?php the_permalink(); ?>" rel=""><?php the_title(); ?></a></h5>
	                        </div>
	                    </header><!-- .entry-header -->
	                    <?php
	                    if($show_description){
				...
				if($count==$start+1){
					echo '<div class="magazine-post-lists">';
				}
			?>
				<article <?php post_class(' post-list-item'); ?>>
                    ...
                    <div class="post-list-content <?php echo $show_list_image?'show-image':'hide-image';?>" <?php echo $show_list_image?'style="padding-left:'.$padding.'px;"':'';?>>
	                    <header class="cs-blog-header">
	                        <div class="cs-blog-meta cs-itemBlog-meta">
	                        	<h6 class="list-title"><a title="<?php the_title(); ?>" href="<?php the_permalink(); ?>" rel="" <?php //echo $item_title_style;?>><?php the_title(); ?></a></h6>
	                            <?php
	                            //echo cshero_title_render();
	                            if($show_list_date):
	                            	$date = ($date_format!='time')?get_the_date($date_format):tp_relative_time(get_the_date('Y-m-d H:i'));
