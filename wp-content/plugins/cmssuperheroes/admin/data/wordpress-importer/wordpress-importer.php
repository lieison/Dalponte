...
<?php if ( ! empty( $this->authors ) ) : ?>
	<h3><?php _e( 'Assign Authors', THEME_NAME ); ?></h3>
	<p><?php _e( 'To make it easier for you to edit and save the imported content, you may want to reassign the author of the imported item to an existing user of this site. For example, you may want to import all the entries as <code>admin</code>s entries.', THEME_NAME ); ?></p>
...
<?php if ( $this->allow_fetch_attachments() ) : ?>
	<h3><?php _e( 'Import Attachments', THEME_NAME ); ?></h3>
	<p>
		<input type="checkbox" value="1" name="fetch_attachments" id="import-attachments" />
		<label for="import-attachments"><?php _e( 'Download and import file attachments', THEME_NAME ); ?></label>
	</p>
<?php endif; ?>

	<p class="submit"><input type="submit" class="button" value="<?php esc_attr_e( 'Submit', THEME_NAME ); ?>" /></p>
</form>
<?php
	}
	...
	function author_select( $n, $author ) {
		...
		if ( $create_users ) {
			if ( $this->version != '1.0' ) {
				_e( 'or create new user with login name:', THEME_NAME );
				$value = '';
			} else {
				_e( 'as a new user:', THEME_NAME );
		...
		if ( ! $create_users && $this->version == '1.0' )
			_e( 'assign posts to an existing user:', THEME_NAME );
		else
			_e( 'or assign posts to an existing user:', THEME_NAME );
